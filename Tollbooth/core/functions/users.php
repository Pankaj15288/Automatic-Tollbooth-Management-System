...
function user_exists($username) {
	...
	$query = mysql_query("SELECT COUNT(`user_id`) FROM `tb` WHERE `username` = '$username'");
	return (mysql_result($query, 0) == 1) ? true : false;
}
...
function user_id_from_username($username) {
	$username = sanitize($username);
	$query = mysql_query("SELECT `user_id` FROM `tb` WHERE `username` = '$username'");
...
function login($username, $password) {
	...
	$username = sanitize($username);
	$password = md5($password);
	$query = mysql_query("SELECT COUNT(`user_id`) FROM `tb` WHERE `username` = '$username' AND `password` = '$password'");
...
function user_detail($vehicle_number, $user_data) {
	...
	$date = date('Y-m-d');
	$time = date('H:i:s');
	mysql_query("INSERT INTO `user_detail` (`user_id`, `tb`, `tb_nh`, `tb_pincode`, `cost`, `updated_balance`, `date`, `time`, `email`) VALUES ('$vehicle_owner_id', '$tb', '$tb_nh', '$tb_pincode', '$cost', '$vehicle_owner_balance', '$date', '$time', '$vehicle_owner_email')");
	$data4 = array();
	$data4['date'] = $date;
	$data4['time'] = $time;
	return $data4;
